...
void server_main(std::atomic_bool* should_run)
{
	...
	Net::IP_Endpoint* client_endpoints =
		(Net::IP_Endpoint*)linear_allocator_alloc(&allocator, sizeof(Net::IP_Endpoint) * c_max_clients);
	...
	float32* time_since_heard_from_clients	=
		(float32*)linear_allocator_alloc(&allocator, sizeof(float32) * c_max_clients);

	Player_Snapshot_State *player_snapshot_states =
		(Player_Snapshot_State*)linear_allocator_alloc(&allocator, sizeof(Player_Snapshot_State) * c_max_clients);
	Player_Extra_State*player_extra_states =
		(Player_Extra_State*)linear_allocator_alloc(&allocator, sizeof(Player_Extra_State)		* c_max_clients);
